<template>
  <Switch v-model="enabled" :class="[enabled ? 'bg-green-600' : 'bg-gray-200', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none']">
...
export default {
    name: 'CSwitch',
    props: { initialStatus: { type: Boolean, default: false }},
  components: {
    Switch,
  },
  setup(props) {
    const enabled = ref(props.initialStatus)
    return {
      enabled,
    }
  },
